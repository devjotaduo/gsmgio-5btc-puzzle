...
const root=path.resolve(__dirname,'..'),out=path.join(root,'_work','checkerboard_token_routes_2026-09-16'),input=JSON.parse(fs.readFileSync(path.join(root,'_work','prime_geometry_2026-09-11','inputs.json'))),sha=b=>crypto.createHash('sha256').update(b).digest();
const rawCases=fs.readFileSync(path.join(out,'cases.jsonl'),'utf8'), complete=rawCases.slice(0,rawCases.lastIndexOf('\n'));
const cases=complete.split('\n').filter(Boolean).map(JSON.parse).filter(c=>c.dataset==='real'),lines=cases.map(c=>JSON.stringify(c));assert.equal(cases.length,140);assert.equal(new Set(cases.map(c=>[c.sourceReversed,c.route].join('/'))).size,140);
...
fs.writeFileSync(path.join(out,'oracle_candidates.json'),JSON.stringify([...candidates.values()])+'\n');fs.writeFileSync(path.join(out,'padding.json'),JSON.stringify(padding)+'\n');
const result={cases:140,uniqueCandidateTexts:candidates.size,passwords:passwords.size,aesAttempts,paddings:padding.length,scalarChecks,pubHits,maxPrintable,mostlyPrintable:padding.filter(p=>p.printable>.9),
  controls:{phase32SHA256:sha(control).toString('hex'),ecGenerator:true},realCasesSHA256:sha(fs.readFileSync(path.join(out,'real_cases.jsonl'))).toString('hex'),candidatesSHA256:sha(fs.readFileSync(path.join(out,'oracle_candidates.json'))).toString('hex'),sourceSHA256:sha(fs.readFileSync(__filename)).toString('hex'),
