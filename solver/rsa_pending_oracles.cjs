'use strict';
// All360 newly compatible DBBI paths and declared FAED witness/extrema sample.
...
function main() {
  assert(read('verification.json').complete && read('candidate_verification.json').complete);
  ...
  const candidates = read('candidates.json').candidates.map(r => r.hex);
  ...
  const result = { complete: true, scope: 'All360 DBBI parse paths plus FAED witness/extrema sample, deduplicated. Five case/spacing forms, direct and SHA256-hex passwords, three blobs, EVP-SHA256 and MD5. SHA256(password), material 32-byte windows and SHA256/windows of PKCS7 bodies in both byte orders. This is a sample of parse paths; zero hits does not exclude untested paths.', inputSHA256: hash(inputRaw).toString('hex'), sourceSHA256: hash(fs.readFileSync(__filename)).toString('hex'), candidates, materials: [...materials.values()], passwords: [...passwords.values()], attempts: flags.length, flags: Buffer.from(flags).toString('base64'), padding, maxPrintable, scalars: valid.length, scalarStreamSHA256: hash(Buffer.concat(valid.map(h => Buffer.from(h, 'hex')))).toString('hex'), matches };
