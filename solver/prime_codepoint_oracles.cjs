'use strict';
// Bounded witness sample: not an enumeration of all accepted parse paths.
...
function main() {
  assert(read('verification.json').complete && read('space_dfa_verification.json').complete);
  ...
  const candidates = [...new Set([...lines('space_dfa_results.jsonl').map(r => r.witnessHex), ...lines('cases.jsonl').filter(r => r.witness).map(r => r.witness.hex)])];
  ...
  for (const [candidate, hex] of candidates.entries()) {
    ...
    for (const [form, value] of [['raw', text], ['lowercase', text.toLowerCase()], ['joinedUppercase', text.replace(/\s/g, '')], ['joinedLowercase', text.replace(/\s/g, '').toLowerCase()]]) {
  ...
  const result = { complete: true, scope: 'One saved witness per compatible configuration plus two generic-search witnesses, deduplicated. Four case/spacing forms, direct and SHA256-hex passwords, three blobs, EVP-SHA256 and MD5. SHA256(password), material 32-byte windows and SHA256/windows of PKCS7 bodies in both byte orders. This is a sample of parse paths; zero hits does not exclude untested paths.', inputSHA256: hash(inputRaw).toString('hex'), sourceSHA256: hash(fs.readFileSync(__filename)).toString('hex'), candidates, materials: [...materials.values()], passwords: [...passwords.values()], attempts: flags.length, flags: Buffer.from(flags).toString('base64'), padding, maxPrintable, scalars: valid.length, scalarStreamSHA256: hash(Buffer.concat(valid.map(h => Buffer.from(h, 'hex')))).toString('hex'), matches };
