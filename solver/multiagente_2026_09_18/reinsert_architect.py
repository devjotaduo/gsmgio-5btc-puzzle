...
def main() -> None:
    ...
    dependencies = [Path(__file__), Path(R.__file__), Path(oracle_module.__file__),
                    ...
                    Path(R.G.O.__file__), Path(R.G.O.README)]
    source_hashes = {source_label(path): R.sha256_file(path) for path in dependencies}
    ...
    inputs = {"dbbi": R.G.DBBI,
              "blobs": {name: {"salt_hex": salt.hex(), "ciphertext_hex": cipher.hex()}
                        for name, (salt, cipher) in R.G.BLOBS.items()}}
    save(out / "inputs.json", inputs)
    spec["inputs_sha256"] = R.sha256_file(out / "inputs.json")
    ...
    if any(R.sha256_file(path) != source_hashes[source_label(path)]
