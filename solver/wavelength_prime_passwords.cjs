'use strict';
// Bounded physical-colour hypothesis; wavelengths are conventional bands,
// not a reconstruction of a monochromatic wavelength from RGB pixels.
const fs = require('node:fs'), path = require('node:path'), crypto = require('node:crypto'), assert = require('node:assert/strict');
const {open} = require('./decimal_keystream_constraints.cjs');
const root = path.resolve(__dirname, '..'), out = path.join(root, '_work/wavelength_primes_2026-09-16');
...
function main() {
  ...
  const blue = Array.from({length: 46}, (_, i) => i + 450).filter(prime), yellow = Array.from({length: 21}, (_, i) => i + 570).filter(prime);
  assert.deepEqual(blue, [457, 461, 463, 467, 479, 487, 491]); assert.deepEqual(yellow, [571, 577, 587]);
  ...
  save('spec.json', {createdAt: new Date().toISOString(), inputSHA256: sha(raw), sourceSHA256: sha(fs.readFileSync(__filename)),
    ...
    hypothesis: 'Assign to blue and yellow prime integer wavelengths in conventional visible-colour bands, then use original row/column sums as password material, directly or SHA256 hexadecimal. Optionally concatenate one of two last-words interpretations.',
    source: {url: 'https://lasp.colorado.edu/wp-content/uploads/2011/06/Wood_monday_SPECTRA.pdf', page: 2, blueNm: [450, 495], yellowNm: [570, 590]},
    ...
    scope: 'The bands are a finite conventional choice, not an RGB-to-spectrum inversion. No frequency units, other colour boundaries, longer concatenations, different sums or untested cryptographic constructions.',
