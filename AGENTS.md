...
(https://gsmg.io/puzzle). O conteúdo central é documentação de pesquisa
colaborativa. Há também scripts experimentais e verificadores em `solver/`,
com controles específicos por campanha; não há build único da aplicação.
O prêmio original de 5 BTC está no
...
- **`README.md`** — o artefato central. Documenta, fase a fase, cada hint, a
  senha/decodificação usada e o resultado.
- **`docs/README.md`** e **`docs/RESEARCH-INDEX.md`** — navegação e índice dos
  relatórios. Leia também `docs/CODEX-NAVIGATION-GUIDE.md`.
- **`ENDGAME.md`** — histórico de hipóteses e limites dos resultados.
- **`solver/`** — scripts e verificadores; ler `solver/README.md` antes de executar.
- **`_work/<experimento>/`** — relatórios, checkpoints e saídas. O `.gitignore`
  mantém os arquivos volumosos locais; não os apague para preparar um commit.
